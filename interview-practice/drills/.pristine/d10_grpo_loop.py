...
def rollout(policy, obs, generator=None):
    ...
    logits = policy(obs)
    weights = logits - logits.amin(dim=-1, keepdim=True) + 1e-6
    actions = torch.multinomial(weights, num_samples=1, generator=generator).squeeze(-1)
...
def compute_advantage(rewards, group_size):
    ...
    adv = (r - r.mean(dim=1, keepdim=True)) / r.std(dim=1, keepdim=True)
    return adv.reshape(-1)


def grpo_loss(logprobs, old_logprobs, advantages, clip_eps=0.2):
    """Clipped surrogate objective."""
    ratio = logprobs - old_logprobs
