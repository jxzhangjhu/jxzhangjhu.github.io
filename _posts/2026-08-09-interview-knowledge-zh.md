...
**Q A1.6.2** — WSD 相比 cosine 解决了什么问题？代价是什么？

Cosine 对着固定总步数定义，第 0 步就把整条 schedule 焊死了。WSD 保留一个恒定的 stable 段，
随时可以从任意一点岔出衰减段——于是「多训一会儿」「从同一个 checkpoint 分叉出数学版和代码版」
「用一次 run 拟合多个算力点」全都变得可行。

代价是：只做一次衰减、算力固定时，最终 loss 通常比 cosine 略差一点。而且注意 WSD **没有**取代
cosine，后者仍然常用。

> **追问**
> - *为什么衰减段这么关键？* → 最后那段衰减里看到的数据，对最终权重的影响大得不成比例。
>   所以最好的数据要留到最后。
> - *WSD 的 loss 曲线为什么看起来吓人？* → stable 段的 loss 一直高于同期 cosine，
>   直到 cooldown 才断崖下跌。中途比较两条曲线会得出错误结论。
...
**Q A1.7.1** — 为什么 Transformer 用 LayerNorm 而不用 BatchNorm？

先说结构差别：BN 沿 batch 轴统计，LN 沿单个 token 的特征轴统计。三个理由都是它的推论。

**一、序列长度可变，且统计量随位置系统性变化。**batch 统计量算在参差不齐的位置集合上，
而位置 1 和位置 500 的分布本就不同——BN 每个特征只有一个 running 估计，对大多数位置都不对。

**二、训练和推理算的是不同的函数。**训练用 batch 统计、推理用 running 统计，
而 NLP 的 batch 统计量在训练中波动极大（比 CV 大几个数量级），两者持续偏离。

**三、分布式下要在噪声和通信之间二选一。**不同步则每卡只有 1–4 条序列的统计量；
用 SyncBatchNorm 则每层每次前向多一次 all-reduce。

LN 三个问题都没有：归约在单个 token 内部，与 batch 组成无关、训练推理同一个函数、
不需要 running 统计量、不需要通信。

> **追问**
> - *那 BN 是不是就不好？* → 不是，视觉里很好用：输入尺寸固定、通道统计稳定、batch 维可交换。
>   这是领域不匹配。
> - *「BN 让 batch=1 推理不了」这个说法对吗？* → **不对**。推理用 running statistics，
>   batch=1 能跑。真正的问题是训练/推理不一致。
>
> **陷阱**
> - 只给一个理由。给三个，并且能说出它们都源自「沿哪个轴统计」这一个区别。
> - 说 BN 让 batch=1 无法推理。
...
> **追问**
> - *LLM 到底会不会过拟合？* → 会。预训练很少过拟合，是因为它接近单 epoch——训练 loss 本身
>   就是在未见数据上算的。但重复超过约 4 个 epoch 会过拟合，SFT 更是高风险区。
> - *那记忆呢？* → 记忆是真实存在的（逐字提取已被证实），但它和泛化不互斥。
>   「不过拟合」说的是 loss 曲线，不是「没背下任何东西」。


**Q A1.8.3** — 训练中 loss 掉到 0 了，怎么解释？如果训练和测试 loss **都**掉到 0 呢？
（Datadog 面到过）
...
**Q A2.1.2** — 什么是 MLM？它的 15% 和 80/10/10 各自为什么是那样？

**MLM** 是 BERT 的预训练目标：随机挑约 15% 的 token 遮住，用**双向**上下文还原它们，
**loss 只在这些位置上算**。

**15% 是个权衡。**太少则一条序列产生的监督太少、训练低效；太多则上下文被破坏、任务无法完成。
它是 BERT 的经验取值而非理论最优——后续工作发现更大模型上用到 40% 也可行。

**80/10/10 是为了修训练/推理错配。**`[MASK]` 在下游根本不出现，如果 100% 用它替换，
模型只会为「带 `[MASK]` 的输入」学表示。掺 10% 随机 token 和 10% 原样不动，
模型无法预知哪个位置会被考，于是被迫对每个位置都建表示。

> **追问**
> - *那「信号少 6 倍」怎么算的？* → $$1/0.15\approx 6.7$$，数的是**每单位数据的预测次数**。
>   但要补一句：MLM 每次预测用的是双向上下文，单次更"值钱"，所以这不是净效率比。
> - *有没有人修过这个浪费？* → ELECTRA，改成判别「每个 token 是否被替换」，
>   每个位置都出监督，同算力下明显优于 BERT。
>
> **陷阱**
> - 把 15% 说成「masked 之后全部替换成 `[MASK]`」。是 80/10/10。
> - 把 6 倍说成信息量之比。它是预测次数之比。
...
**做法二：串行模块（DeepSeek-V3）。**模块之间串起来：第 $$k$$ 个模块的输入既有第 $$k-1$$ 个模块
的表示，**也有 $$t+k$$ 位置真实 token 的 embedding**。于是预测 $$t+2$$ 时是**知道** $$t+1$$ 的，
**完整的因果链被保留下来**，近似的是真正的联合分布而不是独立乘积。
每个模块是一个 transformer block 加一个投影，而且 **embedding 和输出头都和主模型共享**。
V3 取 $$D=1$$，即只多预测一个 token，代价约 2% 的参数量。
...
**MTP 把这三个成本一次性消掉了。**因为那个额外的模块本来就在预测 $$t+2$$：

- 在 decode 的第 $$t$$ 步，一次前向里你**同时**拿到主头给的 $$t+1$$ 和 MTP 模块给的 $$t+2$$ 的提案；
- 下一步做验证：把 $$t+1$$ 喂回去，看主头对 $$t+2$$ 的判断和刚才那个提案是否一致；
- 一致就等于**一次前向产出了两个 token**。

**为什么它的接受率天然就高。**draft 和 target **共享主干、共享 embedding、共享输出头**——
它不是另一个模型对目标分布的近似，它就是同一个模型多伸出来的一只手。
V3 实测第二个 token 的**接受率 85–90%**，换算成吞吐是约 **1.8× TPS**。
...
**三、关于置信度——这里要分成两层，混起来就错了。**

> **常见的错误说法是「calibration 只能靠 post-training」。恰恰相反：
> 基座模型的 token 级校准通常相当好，是 post-training 把它破坏掉的。**
>
> - **Token 级校准来自预训练，而且是目标函数的直接推论。**交叉熵是一个
>   **proper scoring rule**，最小化它就是在把 $$q$$ 推向真实条件分布 $$p$$——
>   而「预测概率等于真实频率」就是校准的定义。所以基座模型在多选题上的
>   概率通常是校准良好的。
> - **口头表达的置信度（verbalized confidence）确实学不到。**因为语料里没有任何监督信号
>   把「模型说自己有 80% 把握」和「模型实际上 80% 的时候是对的」连起来。
>   语料里的人在表达**他们自己**的把握，那和这个模型的知识边界毫无关系。
>   模型学到的是「表达不确定」这个**说话风格**，不是对自身状态的**内省报告**。
>
> 所以准确的说法是：**预训练给你校准的概率，给不了校准的自我报告；
> 而 post-training 会损坏前者，同时是获得后者的唯一途径**（见 A13.3、A13.4）。
> 这个拆分本身就是一个很好的回答——大多数人只会说「RLHF 让模型过度自信」。
...
> 它问的是：给定固定算力 $$C$$，参数量 $$N$$ 和训练 token 数 $$D$$ 该怎么分配才能让 loss 最低？
> 答案是**大致等比例扩展**，$$N\propto C^{0.5}$$、$$D\propto C^{0.5}$$，
> 实用形式是 **$$D \approx 20N$$——每个参数配约 20 个 token**。
...
>
> $$\text{MFU} = \frac{6N\cdot(\text{tokens/s})}{\text{GPU 数}\times\text{峰值 FLOP/s}}$$
>
> 分子用的是**模型所需**的 FLOPs（每 token $$6N$$），不含重算和通信。
...
**第三步：定模型和数据规模。**用 $$C = 6ND$$ 和 $$D \approx 20N$$：
...
$$1.2\times10^{12}/4\times10^6 = 30\text{ 万步}$$——合理。
...
1. **搭一个宽度阶梯**，比如 $$D$$ = 256 / 512 / 1024 的几个小模型，其余配置与目标一致。
...
所以相邻可表示数之间的间隔约为 $$w\times 2^{-8}$$，相对精度约 **0.4%**。
...
（主权重 + $$m$$ + $$v$$）——**这是最大的一块，也是为什么它几乎是白送的**；
ZeRO-2 再切梯度；ZeRO-3 连权重一起切。
...
**数据并行会切分状态，但不会减少激活。**ZeRO 把 16 字节/参数分摊到 $$N_\text{dp}$$ 张卡上，
所以每卡状态是 $$16P/N_\text{dp}$$；而每张卡处理的是**自己那份** micro-batch，
**它的激活一点没少**。

于是：**你每增加一路数据并行，每卡的状态就降一档，而每卡的激活纹丝不动。**
规模越大，激活越是那个绑住你的东西。
...
- **切分状态几乎是免费的。**ZeRO-1 的通信量和 DDP 相当（因为 all-reduce 本来就等于
  reduce-scatter + all-gather），数学上完全等价。
- **减少激活一定要付出代价。**重算换的是约 30% 的额外算力；减小 micro-batch 换的是 MFU；
  上 TP/CP 换的是通信。没有免费的那一档。

> **所以实践中的顺序是：先把 ZeRO 开到够用**（那部分近乎白送），**剩下的就是激活问题**。
> 一个可用的排查顺序：FlashAttention（必开，且不改数学）→ 选择性重算
...
**「RL 装不进新知识，只是重新加权」——这句话该怎么理解。**

**先说机制，这是最硬的一层。**Policy gradient 的更新是
$$\nabla\log\pi_\theta(a\mid s)$$ **乘以 advantage**。所以：

- 一条轨迹要产生梯度，**它必须先被采样出来**；
- 采不出来的东西，$$\pi_\theta$$ 对它的概率就是（数值上的）零，它永远不出现在任何一批 rollout 里；
- 更极端地，如果一道题**所有** rollout 都失败，组内奖励全相同，
  **advantage 精确为零，这一组的梯度就是零**（见 A9.5）。

所以 RL 能做的是**在策略已有的支撑集内部搬运概率质量**，而不是往支撑集外面添东西。
你完全推对了：**完全不会的题，rollout 不出来，advantage 为零，学不会。**
...
把 RLVR 训练后的模型和它的基座模型放在一起，比不同 $$k$$ 下的 pass@$$k$$。

- **小 $$k$$（比如 pass@1）：RL 模型明显更好**——这正是我们要的。
- **大 $$k$$（比如 pass@256）：基座模型反而更好。**

这个交叉是关键。如果 RL 真的学会了新东西，它在任何 $$k$$ 下都不该输给基座。
输了，说明**基座本来就能解出那些题**，只是需要多采几次；而 RL 把概率质量集中到了少数几条路径上，
**采样效率上去了，覆盖面下来了**。他们进一步做了覆盖率和困惑度分析，
确认 RL 模型产出的推理路径**本来就在基座的采样分布里**。
...
**RL 装不进的是「知识」，但它确实能重塑「行为」。**基座模型不知道的事实，RL 变不出来。
但**什么时候该验算、什么时候该回溯、想多久才停**——这些是**轨迹层面的策略**，
不是事实，而奖励恰恰能为它们提供信号。R1-Zero 里自发长出的自我检查和回溯就是这一类：
那些 token 基座本来就产得出来，RL 让它们变成了**系统性的行为**。

**一句话总结给面试用：**

> **能力从预训练、midtrain、SFT、蒸馏进来；RL 把「有时候能做对」变成「稳定做对」，
> 并且重塑轨迹层面的行为。它移动概率质量，不扩充支撑集。**

这个理解还直接推出两个实践结论：**prompt 必须选在 50% 成功率附近**（两端梯度都为零，见 A9.5），
以及**小模型上要先蒸馏再 RL**，否则成功率贴地、RL 根本没有信号可用。
...
>
> **追问**
> - *有没有证据？* → pass@k 的交叉：RL 模型在 pass@1 上更好，基座在 pass@256 上更好，
>   说明那些解本来就在基座的分布里（Yue 等 2025）。
> - *那 R1-Zero 长出来的自我检查算不算新能力？* → 算「新行为」，不算「新知识」。
>   那些 token 基座本来就产得出来，RL 让它们变得系统化。
> - *这个说法有争议吗？* → 有。另一派认为收缩是训练过早终止导致的，延长训练后可能出现扩展。
>   说「目前的 RLVR 主要是在提升采样效率」比说死更稳妥。
>
> **陷阱**
> - 说 RL 能"教会模型新知识"。它移动概率质量，不扩充支撑集。
> - 反过来说死成「RL 完全没用」。它把 pass@1 拉上去，而**产品要的正是 pass@1**。
...
影响超常，所以你想把最好的数据放在最后。这也是 WSD 调度流行起来的原因——有一段恒定的
